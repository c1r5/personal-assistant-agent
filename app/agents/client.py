...
class AgentClient:
    ...
    async def create_session(self, user_id: str):
        ...
        session = await self.__runner.session_service.create_session(
    ...
    async def delete_session(self, user_id: str):
        ...
        await self.__runner.session_service.delete_session(
            app_name=APP_NAME, session_id=self.__sessions[user_id].id, user_id=user_id
        )
    ...
    async def request(self, user_id: str, message: str):
        ...
        try:
            ...
            async for response in self.__runner.run_async(
                user_id=user_id, session_id=session.id, new_message=UserContent(message)
            ):
